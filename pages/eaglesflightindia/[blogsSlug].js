...
export async function getServerSideProps(context) {
    const post5 = await fetch('https://eaglesflightindiablog.vercel.app/api/posts')
    const posts5 = await post5.json()

    const cat = await fetch('https://eaglesflightindiablog.vercel.app/api/categories')
    const cats = await cat.json()


    const posturl = context.params.blogsSlug;
    const res = await fetch(`https://eaglesflightindiablog.vercel.app/api/post/${posturl}`)
    const posts = await res.json()

    const getfeedback = await fetch(`https://eaglesflightindiablog.vercel.app/api/posts/feed/${posturl}`)
...
const submitFeedback = async (event) => {
    ...
    xhr.open('POST', 'https://eaglesflightindiablog.vercel.app/api/feedback/');
    ...
    return (
        <>
            <Head>
                ...
                <link rel="canonical" href={"https://blog.byldgroup.com/eaglesflightindia/"+posts.posturl} />
                {/* Twitter */}

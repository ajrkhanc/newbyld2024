...
export async function getServerSideProps(context) {
    const post5 = await fetch('https://clblogs.vercel.app/api/posts')
    ...
    const res = await fetch(`https://clblogs.vercel.app/api/post/${posturl}`)
    const posts = await res.json()

    const getfeedback = await fetch(`https://clblogs.vercel.app/api/posts/feed/${posturl}`)
...
const submitFeedback = async (event) => {
    ...
    xhr.open('POST', 'https://clblogs.vercel.app/api/feedback/');

import Head from "next/head";
import Link from "next/link";
import Slider from "react-slick";

export default function ClientCorner() {
  return (
    <>
      <Head>
        <title>Sitemap - BYLD Group</title>
        <meta
          name="description"
          content="Continuously adding value to our customer’s businesses! Continuously adding value to our customer’s businesses! Let us help you find the best resource for your company. Highly effective, employee screening process for hiring of blue-collared and entry-level staff. Highly effective, employee screening process for hiring of blue-collared and entry-level staff. Business Enquiry Submit Your Query Our… Continue reading Client Corner"
        />
      </Head>

      <div className="ptt-50 pbb-50">
        <div className="container custom">
          <div className="row y-middle rowbor">
            <div className="col-md-12">
              <div className="sitemap1">
                <h1>Our Solutions</h1>
              </div>
              <div className="row sitemaprow">
                <div className="col-sm-4">
                  <ul>
                    <li>
                      <a href="/our-solutions/experiential-learning">
                        Experiential Learning
                      </a>
                    </li>
                    <li>
                      <a target="_blank" href="/eaglesflightindia/">
                        Eagle’s Flight™
                      </a>
                    </li>
                    <li>
                      <a href="/business-today-simulations">
                        Business Today Simulations
                      </a>{" "}
                    </li>
                    <li className="d-none">
                      <a href="#">Gaminar</a>
                    </li>
                    <li>
                      <a href="/jenson-8">
                        Jenson 8 (Virtual Reality Trainings)
                      </a>
                    </li>
                  </ul>
                </div>
                <div className="col-sm-4">
                  <ul>
                    <li className="">
                      <a href="/our-solutions/leadership-and-talent-development">
                        Leadership & Performance
                      </a>
                    </li>
                    <li>
                      <a
                        target="_blank"
                        href="https://blanchardinternational.co.in/"
                      >
                        Blanchard India
                      </a>
                    </li>
                    <li>
                      <a target="_blank" href="/cruciallifechangingskills/">
                        Crucial Life Changing Skills
                      </a>
                    </li>
                    <li>
                      <a href="/tirian">Tirian</a>
                    </li>
                    <li>
                      <a href="/leadership-and-performance/trapologist">
                        Trapologist at Work™
                      </a>
                    </li>
                  </ul>
                </div>
                <div className="col-sm-4">
                  <ul>
                    <li className="">
                      <a href="/our-solutions/assessments">Assessments</a>
                    </li>
                    <li>
                      <a target="_blank" href="/everythingdisc">
                        Everything DiSC (A Wiley Brand)
                      </a>{" "}
                    </li>
                    <li>
                      <a target="_blank" href="/fivebehaviors">
                        Five Behaviors
                      </a>{" "}
                    </li>
                    <li>
                      <a href="/assessments/lumina-psychometric-assesments">
                        Lumina Spark
                      </a>
                    </li>
                    <li>
                      <a href="/assessments/talentsmart-eq-assesments">
                        TalentSmart EQ
                      </a>
                    </li>
                    <li className="d-none">
                      <a href="/assessments/assessments-and-development-centers">
                        Assessments and Development Centers
                      </a>
                    </li>
                  </ul>
                </div>
                <div className="col-sm-4">
                  <ul>
                    <li className="">
                      <a href="/bespoke">BeSpoke</a>
                    </li>
                    <li>
                      <a href="/bespoke/persona-global">Persona Global</a>{" "}
                    </li>
                    <li className="d-none">
                      <a href="#">SLI</a>
                    </li>
                    <li className="d-none">
                      <a href="#">Large Projects</a>
                    </li>
                  </ul>
                </div>
                <div className="col-sm-4">
                  <ul>
                    <li className="">
                      <a href="/our-solutions/coaching">Coaching</a>
                    </li>
                    <li>
                      <a href="/coaching/become-a-coach">ICF Certification</a>
                    </li>
                    <li>
                      <a href="/coaching/insideout-coaching">
                        InsideOut™ Coaching
                      </a>
                    </li>
                    <li>
                      <a href="/coaching/breakthroughs">Breakthroughs</a>
                    </li>
                    <li>
                      <a href="/coaching/executive-coaching">
                        Executive Coaching
                      </a>
                    </li>
                  </ul>
                </div>
                <div className="col-sm-4">
                  <ul>
                    <li className="">
                      <a href="/our-solutions/staffing">Staffing & Search</a>
                    </li>
                    <li>
                      <a
                        target="_blank"
                        href="https://www.yomabusinesssolutions.com/"
                      >
                        YOMA Business Solutions
                      </a>{" "}
                    </li>
                    <li className="">
                      <a
                        target="_blank"
                        href="https://www.yomatechnologies.in/"
                      >
                        YOMA Technologies
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
          <br></br>
          <div className="row y-middle rowbor">
            <div className="col-md-12">
              <div className="sitemap1">
                <h1>Others</h1>
              </div>
              <div className="row sitemaprow">
                <div className="col-sm-3">
                  <ul>
                    <li className="">
                      <a href="/">HOME</a>
                    </li>
                    <li>
                      <a href="/client-corner">CLIENT CORNER</a>
                    </li>
                    <li>
                      <a href="/blog">BLOG</a>
                    </li>
                    <li className="">
                      <a href="/contact-us">CONTACT US</a>
                    </li>
                  </ul>
                </div>

                <div className="col-sm-3">
                  <ul>
                    <li className="menu-item-has-children">
                      <a href="/about-us">ABOUT US</a>
                    </li>
                    <li>
                      <a href="/mission-vision-values">
                        Mission, Vision and Values
                      </a>
                    </li>
                    <li>
                      <a href="/our-leadership-team">Our Leadership Team</a>
                    </li>
                  </ul>
                </div>
                <div className="col-sm-3">
                  <ul>
                    <li className="menu-item-has-children">
                      <a href="#">EVENTS</a>
                    </li>
                    <li>
                      <a href="/webinars-and-program-experience">Webinars</a>
                    </li>
                    <li>
                      <a href="/workshops">Workshops</a>
                    </li>
                  </ul>
                </div>
                <div className="col-sm-3">
                  <ul>
                    <li className="menu-item-has-children">
                      <a href="#">MEDIA</a>
                    </li>
                    <li>
                      <a href="/press">BYLD Group in News</a>
                    </li>
                    <li>
                      <a href="/events-gallery">Events</a>
                    </li>
                    <li>
                      <a href="/awards-accolades">Awards & Accolades</a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </>
  );
}
